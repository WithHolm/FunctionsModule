function ConvertTo-FunStorageTableEntity {
    [CmdletBinding()]
    ...
    process {
        "rowkey","partitionkey"|%{
            $tests = Test-FunStorageTableKey -Key $InputItem.$_
            if($tests.count -gt 0)
            {
        ...
        else
        {
            ...
            {
                Set-Warning -tag 'WarnConvertType' -message "The inputitem is of type $($InputItem.gettype()), and i cannot guarantee full support. it might work, but please check output."
            }
...
# @{test=1;partitionkey=1;rowkey="r"}|ConvertTo-FunStorageTableEntity -Verbose