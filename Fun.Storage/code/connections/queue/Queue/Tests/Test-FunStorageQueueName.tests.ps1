describe "Test-FunStorageQueueName" -tag 'cmdlet','queue'{
    InModuleScope "Fun.Storage"{
        ...
        it "should <type> if name has <desc>" -TestCases $tests -Test{
            ...
            if($type -eq 'fail')
            {
                $Validate = (Test-FunStorageQueueName -Name $name)
                $Validate.count |should -be 1 -Because "should only have one error: $($Validate -join ', ')"
            }
            else {
                (Test-FunStorageQueueName -Name $name).count |should -BeExactly 0
            }
        }
    }
}