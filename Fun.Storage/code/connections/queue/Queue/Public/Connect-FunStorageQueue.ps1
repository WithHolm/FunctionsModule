...
function Connect-FunStorageQueue {
    [CmdletBinding()]
    ...
    process {
        ...
        {
            $client = New-FunstorageQueueClient
        }
        else {
            $client = New-FunstorageQueueClient -Connectionstring $Connectionstring
        }
        Write-Verbose "connected with client '$($client.BaseUri)'"


        $Queue = Get-FunStorageQueue -Client $client -Name $Name

        if($CreateIfNotExist -and [string]::IsNullOrEmpty($Queue))
        {
            $Queue = New-FunStorageQueue -Client $client -Name $Name
        }
